...
class WifiACGuardianWinUI(tk.Tk):
    ...
    def __init__(self, config: Optional[GuardianConfig] = None, guardian: Optional[WifiACGuardianWin] = None):
        ...
        # Start compact and centered; users can still maximize when they need more room.
        screen_width = self.winfo_screenwidth()
        screen_height = self.winfo_screenheight()
        window_width = min(900, max(820, int(screen_width * 0.72)))
        window_height = min(720, max(620, screen_height - 130))
    ...
    def _build_ui(self) -> None:
        ...
        for key, icon, title, value in [
            ("status", "shield", "Status", "Protected"),
            ("tx", "bolt", "TX rate", "— Mbps"),
            ("rx", "bolt", "RX rate", "— Mbps"),
            ("retry", "history", "Retry state", "0 / 99"),
        ]:
            card_shell = RoundedCard(kpi_strip, surface=COLOR_CARD, radius=12, inset=10)
            card_shell.pack(side="left", fill="both", expand=True, padx=3)
        ...
        grid_cols = [("Connected to", "lab5g", "connected_to"), ("Status", "GOOD", "status_val"),
                     ...
                     ("Signal", "95%", "signal_val")]
        self.hero_labels = {}
        for label_k, default_v, key in grid_cols:
        ...
        self._section_header(left_card, "wifi", "Connection overview")
        overview_grid = tk.Frame(left_card, bg=COLOR_CARD)
        overview_grid.pack(fill="both", expand=True, pady=(8, 0))
        self.overview_labels = {}
        pairs_left = [("SSID", "lab5g", "ssid"), ("PHY mode", "802.11ac (Wi-Fi 5)", "phy"),
                      ("Speed", "780 Mbps", "speed"), ("Signal", "95%", "signal"),
                      ("Interface", "Wi-Fi", "interface"), ("Frequency", "5805 MHz", "freq"),
                      ("Channel", "161 (5 GHz)", "channel"), ("Retry attempts", "0", "retries")]
        for idx, (k_txt, v_txt, key) in enumerate(pairs_left):
            r, c = divmod(idx, 2)
            cell = tk.Frame(overview_grid, bg=COLOR_CARD)
            cell.grid(row=r, column=c, sticky="ew", padx=(0, 6), pady=4)
        ...
        self.engine_labels = {}
        pairs_right = [("Check interval", "10 sec", "interval"),
                       ("Reconnect delay", "15 sec", "delay"), ("Retry attempts", "99 (Auto)", "attempts"),
                       ("Last check", "Just now", "last_check")]
        for idx, (k_txt, v_txt, key) in enumerate(pairs_right):
            r, c = divmod(idx, 2)
            ...
            tk.Label(cell, text=k_txt, font=(FONT_UI, 8), fg=COLOR_TEXT_MUTED, bg=COLOR_CARD).pack(anchor="w")
            v_fg = COLOR_TEXT_PRIMARY
            v_lbl = tk.Label(cell, text=v_txt, font=(FONT_MONO, 8, "bold"), fg=v_fg, bg=COLOR_CARD)
            v_lbl.pack(anchor="w", pady=(2, 0))
        ...
        toolbar.pack(side="bottom", fill="x", padx=28, pady=(0, 20))
    ...
    def _update_ui(self, link: LinkInfo) -> None:
        ...
        if not link.connected:
            ...
            self.overview_labels["ssid"].config(text="None")
            self.overview_labels["phy"].config(text="Disconnected")
            self.overview_labels["speed"].config(text="0.0 Mbps")
            self.overview_labels["signal"].config(text="N/A")
            self.overview_labels["interface"].config(text=link.interface or "Wi-Fi")
            self.overview_labels["freq"].config(text="N/A")
            self.overview_labels["channel"].config(text="N/A")
        ...
        # Update Connection Overview Grid
        self.overview_labels["ssid"].config(text=link.ssid or "Unknown")
        self.overview_labels["phy"].config(text=link.phy_summary)
        self.overview_labels["speed"].config(text=bitrate_txt)
        self.overview_labels["signal"].config(text=sig_txt)
        self.overview_labels["interface"].config(text=link.interface)

        self.overview_labels["freq"].config(text=link.radio_type or "802.11ac")
        self.overview_labels["channel"].config(text="5 GHz (Auto)")
        self.overview_labels["retries"].config(text=f"{self.guardian.state.attempts_count} / {self.config.max_attempts}")
